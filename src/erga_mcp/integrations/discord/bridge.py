...
def _backend_prompt(message: str) -> str:
    return (
        ...
        "For every resume edit, make the smallest change the user requested: preserve every "
        "unmentioned section, project, bullet, template macro, margin, spacing rule, and inline "
        "emphasis byte-for-byte. Never add research, projects, claims, formatting changes, or "
        "layout surgery merely because they seem helpful. A master-only reorder or constrained "
        "edit does not require a job description. Do not inspect repositories or alternate web "
        "sources unless the user explicitly asks for research. If the exact request cannot be "
        "supported by approved evidence, stop once and state the single missing fact; do not "
        "propose a chain of follow-up work. Never claim a file changed unless the Erga tool "
        "result confirms the saved artifact and its validation. "
...
def _record_matches_process(record: DiscordProcessRecord) -> bool:
    ...
    return bool(
        command
        and any(module_path in command for module_path in _BRIDGE_MODULE_PATHS)

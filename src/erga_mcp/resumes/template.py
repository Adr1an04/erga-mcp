...
def _is_tailorable_latex_master(master: ResumeSource) -> bool:
    """Return whether Erga can edit the source without reconstructing its visual design."""
    ...
    return all(marker in master.text for marker in required)


def _render_master_latex_template(master: ResumeSource) -> str:
    """Keep a real LaTeX master byte-for-byte beneath private provenance comments."""
    return (
        f"% Generated by Erga from approved master resume SHA-256: {master.sha256}\n"
        "% Factual text and visual formatting below come from that approved master source.\n"
        f"{_SEMANTIC_TEMPLATE_MARKER}\n"
        f"{_VISUAL_SPACING_MARKER}\n"
        f"{master.text.rstrip()}\n"
    )
...
def generate_latex_template(
    ...
    *,
    ...
    rendered = (
        ...
        else _render_template(master, style)
    )
    ...
    if "Projects" in profile.editable_sections:
        # A real LaTeX master or explicit style owns its observed number of project slots.
        # Reconstructed defaults start at four and let rendered packing decide bullet density.
        explicit_style_count = (
            _observed_project_count(master)
            if preserves_master_latex
            else _observed_project_count(style)
    ...
    metadata = {
        ...
        "styling_source": (
            ...
            if style
            else "erga-default-jake"
        ),
        "master_latex_preserved": preserves_master_latex,

...
def _adapt_project_heading_structure(source: str, contract: _ProjectHeadingContract) -> str:
...
def _project_heading_contract_issues(
    ...
    for index, heading in enumerate(_project_entry_headings(proposed_section), start=1):
        ...
        compatible_mode = mode == contract.mode or (
            mode == "title_only" and contract.mode in {"inline", "right_technology"}
        )
    ...
    for requested_name, tailorer in (
        ...
        if requested_name == "Projects" and project_candidates:
            ...
            inventory_section = (
                prefix
                + "".join(
                    _adapt_project_heading_structure(item.latex, heading_contract)

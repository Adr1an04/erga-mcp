...
def _compact_generated_entry_section(
    ...
    *,
    ...
    if optimize_across_entries:
        ranked_entries: list[tuple[int, str, list[_RankedValue], int]] = []
        content_index = 0
        for entry_index, entry in enumerate(entries):
            ...
            content_index += 1
            ranked_entries.append((entry_index, entry, ranked, max(0, cap)))
        counts = {entry_index: 0 for entry_index, _, _, _ in ranked_entries}
        remaining = maximum_items
        # Preserve breadth first: give the strongest roles one bullet before allocating depth.
        for entry_index, _, ranked, cap in sorted(
            ranked_entries,
            key=lambda item: (
                -max((bullet.score for bullet in item[2]), default=0),
                item[0],
            ),
        ):
            if remaining <= 0:
        ...
        while remaining > 0:
            choices = [
                ...
                for entry_index, _, ranked, cap in ranked_entries
...
def apply_adaptive_single_page_fill(source: str) -> str:
    ...
    for item_list in _ITEM_LIST_END_LINE.finditer(document):
        additions.append(
            (
                document_start + item_list.end(),
                f"\n{item_list.group('indent')}\\vspace{{0pt plus 1fill}}% {_PAGE_FILL_MARKER[2:]}",
            )
        )

...
# The master is the user-approved factual source; template_path is an optional editable output base.
master_path = ""
template_path = ""
# Optional existing resume used only as a formatting and length reference.
reference_path = ""
editable_sections = []
bullet_min_chars = 0
...
# Choose career, read, research, write, or hermes for a narrower MCP client surface.
...
def load_config(config_path: Path) -> ErgaConfig:
    ...
    if mcp_tool_profile not in {"career", "default", "read", "research", "write", "hermes"}:
        raise ValueError(
            "mcp tool_profile must be career, default, read, research, write, or hermes"
        )

...
def _tailor_skills(
    ...
    *,
    ...
    def tailored_values(category: str, values: str) -> str:
        ...
        for ranked_index, (original_index, value, score, matched) in enumerate(ranked):
...
def apply_adaptive_single_page_fill(
    source: str, *, additional_space_pt: float | None = None
) -> str:
    """Distribute spare page height between semantic groups without changing résumé copy."""
    if _PAGE_FILL_MARKER in source:
        return source
    if source.count(r"\resumeItem{") < 6:
        return source
    if additional_space_pt is not None and additional_space_pt <= 0:
        return source
    ...
    insertion_points: list[tuple[int, str]] = [
        (document_start + section.start(), section.group("indent")) for section in sections[1:]
    ]
    insertion_points.extend(
        (document_start + item_list.end(), item_list.group("indent"))
        for item_list in _ITEM_LIST_END_LINE.finditer(document)
    )
    if not insertion_points:
        return source
    if additional_space_pt is None:
        spacer = r"\vspace{0pt plus 1fill}"
        setup = _PAGE_FILL_SETUP
    else:
        spacer = rf"\vspace{{{additional_space_pt / len(insertion_points):.2f}pt}}"
        setup = f"\n{_PAGE_FILL_MARKER}\n"
    additions: list[tuple[int, str]] = []
    section_count = len(sections[1:])
    for index, (offset, indent) in enumerate(insertion_points):
        prefix = "" if index < section_count else "\n"
        suffix = "\n" if index < section_count else ""
        additions.append((offset, f"{prefix}{indent}{spacer}% {_PAGE_FILL_MARKER[2:]}{suffix}"))
    for offset, addition in sorted(additions, key=lambda item: item[0], reverse=True):
        source = source[:offset] + addition + source[offset:]
    return source.replace(document_marker, document_marker + setup, 1)
...
def create_automatic_resume_proposal(
    *,
    ...
    page_fill_extra_pt: float | None = None,
    max_pages: int = 0,
    ...
    for requested_name, tailorer in (
        ...
        if requested_name == "Projects" and project_candidates:
            ...
            inventory_section = _separate_legacy_project_technology_stacks(
                prefix + "".join(item.latex for item in selected_projects) + suffix
            )
    ...
    if minimum_page_fill_ratio:
        proposed = apply_adaptive_single_page_fill(proposed, additional_space_pt=page_fill_extra_pt)

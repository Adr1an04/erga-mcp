...
""".lstrip(),
    ...
    def test_generated_template_packer_backfills_stored_projects_past_style_budget(self) -> None:
        with TemporaryDirectory() as directory:
            root = Path(directory)
            resume = root / "resume.tex"
            resume.write_text(
...
\resumeItemListStart
\resumeItem{Built placeholder one.}
\resumeItem{Tested placeholder one.}
\resumeItemListEnd
\resumeProjectHeading{\textbf{Placeholder Two}}{}
\resumeItemListStart
\resumeItem{Built placeholder two.}
\resumeItem{Tested placeholder two.}
\resumeItemListEnd
\end{itemize}
\end{document}
""".lstrip(),
                ...
                json.dumps(
                    {
                        "style_sha256": "d" * 64,
                        "style_layout_profile": {
                            "section_entry_item_counts": {"Projects": [2, 2]},
                            "section_item_counts": {"Projects": 4},
                        },
                        "layout_profile": {"section_entry_item_counts": {"Projects": [2, 2]}},
                    }
                ),
                encoding="utf-8",
            )
            ...
            evidence = Evidence(
                "ev_project_fill",
                "approved:project-fill",
                "Approved synthetic project evidence.",
                True,
                datetime.now(UTC),
            )
            verbs = (
                ("Engineered", "Reduced", "Validated", "Shipped"),
                ("Architected", "Automated", "Hardened", "Accelerated"),
            )

            def candidate(index: int) -> ProjectCandidate:
                project_id = f"project-{index + 1}"
                bullets = tuple(
                    f"{verb} approved {project_id} systems with verified outcome {offset + 1}."
                    for offset, verb in enumerate(verbs[index])
                )
                return ProjectCandidate(
                    ...
                    bullet_evidence_ids=tuple((evidence.id,) for _ in bullets),
                    tags=("systems",),
                )
            ...
            with patch(
                ...
                side_effect=lambda **kwargs: (
                    True,
                    kwargs["section_item_limits"].get("Projects", 0) / 10,
                    (),
                ),
            ) as trial:
                result = _create_render_packed_automatic_resume_proposal(
                    resume_path=resume,
                    output_dir=root / "artifacts",
                    job_description="systems project engineering",
                    evidence=[evidence],
                    project_candidates=(candidate(0), candidate(1)),
                    config=config,
            ...
            proposed = result.proposal.proposed_tex_path.read_text(encoding="utf-8")
            packing = cast(dict[str, Any], result.project_selection["rendered_content_packing"])
            self.assertTrue(trial.called)
            self.assertEqual(proposed.count(r"\resumeItem{"), 8)
            self.assertEqual(packing["section_entry_item_limits"]["Projects"], (4, 4))
            self.assertEqual(packing["style_reference_item_budget"], 4)
...
""".lstrip(),
            ...
            config_path.write_text(
                DEFAULT_CONFIG.replace(
                    "minimum_page_fill_ratio = 0.90", "minimum_page_fill_ratio = 0"
                ).replace(
                    "editable_sections = []",
...
""".lstrip(),
    ...
    def test_project_bullet_density_stops_at_first_balanced_filled_tier(self) -> None:
        with TemporaryDirectory() as directory:
            ...
            self.assertEqual(fill_ratio, 0.92)
            self.assertEqual(trial.call_count, 2)

    def test_project_bullet_density_can_exceed_visual_template_budget_to_fill_page(self) -> None:
        with TemporaryDirectory() as directory:
            root = Path(directory)
            resume = root / "resume.tex"
            ...
            with patch(
                "erga_mcp.mcp.server._project_density_trial",
                side_effect=[(True, 0.60), (True, 0.74), (True, 0.88)],
            ) as trial:
                selected, requires_spacing, fill_ratio = _select_rendered_project_bullet_density(
                    resume_path=resume,
                    job_description="Python systems",
            ...
            self.assertEqual(fill_ratio, 0.88)
...
""".lstrip(),
    ...
    def test_resume_validation_tool_enforces_configured_page_fill(self) -> None:
        with TemporaryDirectory() as directory:
            ...
            self.assertEqual(compile_proposal.call_args.kwargs["minimum_page_fill_ratio"], 0.90)

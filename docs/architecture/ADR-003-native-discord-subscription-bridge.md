...
to adopt and duplicates capabilities the user's coding tool already provides.
...
- Non-secret settings contain an explicit project directory and Discord user-ID allowlist.
- Server messages require a bot mention by default.
- Ambient OpenAI and Anthropic model API keys are removed from the corresponding child process.

...
4. **Résumé tailoring:** a source-backed role profile separates required, preferred, responsibility, and contextual signals. Conservative aliases improve ranking without approving facts. Approved evidence is indexed as stable claim-sized records; matching ranks experience, skills, and the project catalogue with requirement explanations. Authenticated Git changes broaden the project shortlist, and optional MCP client sampling synthesizes project bullets with project-scoped evidence IDs. Unsupported numbers, technologies, claim atoms, cross-project citations, internal Git accounting, unsafe LaTeX, and rendered overflow are rejected deterministically. Lead-verb uniqueness is an explicit style preference rather than a factual invariant. The result is a proposed LaTeX patch, claim/decision reports, readiness status, and PDF compile result; clients without sampling retain the deterministic approved-copy path.

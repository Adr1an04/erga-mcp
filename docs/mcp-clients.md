...
| --- | --- |
| `career` | Bounded job intake and document workflow, plus local-only search of an explicitly enabled Keryx public-data cache. It excludes bulk private-data export, full writing-style source context, mail, Hermes monitors, Git scanning, and token recording. |
...
| `read` | Local read-only records, tracker, token summary, and optional Keryx cache search. It deliberately excludes writing-sample/template content. |
